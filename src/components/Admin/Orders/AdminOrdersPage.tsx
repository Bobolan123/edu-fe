...
function RevenueDisplay({ revenue }: { revenue: number }) {
  ...
  const [formattedRevenue, setFormattedRevenue] = useState<string>(currencyService.formatPrice(revenue, 'VND'));

  useEffect(() => {
    const formatRevenue = async () => {
      try {
        if (currency === 'USD') {
          const convertedRevenue = await currencyService.convertPrice(revenue, 'VND', 'USD');
          setFormattedRevenue(currencyService.formatPrice(convertedRevenue, currency));
        } else {
          setFormattedRevenue(currencyService.formatPrice(revenue, 'VND'));
        }
      } catch (error) {
...
export default function AdminOrdersPage({ 
  ...
  const [selectedPayment, setSelectedPayment] = useState(searchParams.paymentMethod || '');
