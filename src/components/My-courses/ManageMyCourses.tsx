...
export default function ManageMyCourses({ courses }: IManageMyCoursesProps) {
  ...
  const totalRevenue = courses?.reduce(
    (sum, course) => sum + Number(course.price || 0) * (course.enrollments?.length || 0),
    0
  ) || 0;
  const averageRating =
        ...
        <Container maxWidth="xl">
          <div className="flex flex-col gap-4 mb-8">
            <div className="flex items-center justify-between">
              <div>
                <Typography variant="h3" component="h1" className="font-bold tracking-tight mb-2">
                  My Courses
                </Typography>
              ...
              <Button variant="contained" startIcon={<Plus className="w-4 h-4" />} sx={{ textTransform: "none" }}>
                Create New Course
              </Button>
            </div>
                ...
                <Card className="h-full">
                  <CardContent className="p-6">
                    ...
                    <Typography variant="caption" color="text.secondary">
                      {stat.label === "Avg. Rating" ? "Across all courses" : `For ${stat.label.toLowerCase()}`}
                    </Typography>
                  </CardContent>
                </Card>
              </Grid>
              ...
              <Grid item xs={12} md={6} lg={4} key={course.id}>
                <Card className="h-full hover:shadow-lg transition-shadow cursor-pointer">
                  ...
                  <CardContent className="p-4">
                    <Typography variant="h6" component="h3" className="font-semibold line-clamp-2">
                      {course.title}
                    </Typography>
                    ...
                    <Box className="flex items-center justify-between text-sm pt-2">
                      <Box className="flex items-center gap-1">
                        <Rating value={course.average_rating} precision={0.1} size="small" readOnly />
                      ...
                      <Box className="flex items-center gap-1 text-gray-500">
                        ...
                        <Typography variant="caption">{course.duration}h</Typography>
                      </Box>
                    </Box>
                    ...
                    <Box className="flex gap-2 pt-3">
                      <Button variant="contained" size="small" className="flex-1" sx={{ textTransform: "none" }}>
                        Manage Course
                      </Button>
                      <Button variant="outlined" size="small" sx={{ textTransform: "none" }}>
                        Analytics
          ...
          >
            ...
            <MenuItem onClick={handleMenuClose}>Manage Enrollments</MenuItem>
            <MenuItem onClick={handleMenuClose}>View Reviews</MenuItem>
